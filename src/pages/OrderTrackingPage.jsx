...
import { Search, MapPin, Truck, CheckCircle2, Box, Package, QrCode } from 'lucide-react';
import { useOrders } from '../context/OrderContext';

const OrderTrackingPage = () => {
    const [searchQuery, setSearchQuery] = useState('');
    const { orders, cancelOrder } = useOrders();
    const [activeOrder, setActiveOrder] = useState(orders[0]); // Default to first order for demo

    const handleSearch = (e) => {
        e.preventDefault();
        const found = orders.find(o => o.id.includes(searchQuery) || o.id === searchQuery);
            ...
            <div className="bg-stone-900 border border-white/5 rounded-[2.5rem] p-10 shadow-2xl space-y-12">
                ...
                {activeOrder.status === 'Cancelled' ? (
                    <div className="bg-rose-500/10 border border-rose-500/20 rounded-2xl p-10 text-center space-y-4">
                        <div className="w-20 h-20 bg-rose-500 rounded-full flex items-center justify-center mx-auto text-white shadow-lg shadow-rose-500/20">
                            <CheckCircle2 size={40} className="rotate-45" /> {/* Using rotate as a visual 'X' or similar */}
                        </div>
                        ...
                        <p className="text-accent-cream/60 italic max-w-md mx-auto">
                            Your order has been cancelled by the boutique. An automatic refund of **{activeOrder.amount}** has been processed through your original payment method.
                        </p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-8 relative pb-10">
                        ...
                        {steps.map((step, i) => (
                            <div key={i} className="flex flex-col items-center text-center space-y-4 relative z-10">
                                ...
                                <div>
                                    <p className={`text-sm font-bold uppercase tracking-widest ${step.status === 'upcoming' ? 'text-accent-cream/20' : 'text-accent-cream'
                                        }`}>{step.label}</p>
                ...
                <div className="bg-accent-gold/5 rounded-2xl p-8 border border-accent-gold/10 space-y-4">
                    ...
                    <p className="text-xs text-accent-cream/60 leading-relaxed italic">
                        "Your artisanal selection is assigned a unique **Delivery Code**. This code must be presented to our delivery partner for scanning upon arrival. Verification will instantly update your shipment to 'Delivered' in our system."
                    </p>
                </div>
            </div>
